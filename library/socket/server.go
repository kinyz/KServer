...
type Server struct {
	...
	msgHandler isocket.IMsgHandle
	//当前Server的链接管理器
	ConnMgr isocket.IConnManager
	//该Server的连接创建时Hook函数
	OnConnStart func(conn isocket.IConnection)
	//该Server的连接断开时的Hook函数
	OnConnStop func(conn isocket.IConnection)
}

/*
  创建一个服务器句柄
*/
func NewSocket() isocket.IServer {
...
func (s *Server) AddHandle(id uint32, handle isocket.IHandle) {
...
func (s *Server) SetOnConnStart(hookFunc func(isocket.IConnection)) {
	s.OnConnStart = hookFunc
}

//设置该Server的连接断开时的Hook函数
func (s *Server) SetOnConnStop(hookFunc func(isocket.IConnection)) {
	s.OnConnStop = hookFunc
}

//调用连接OnConnStart Hook函数
func (s *Server) CallOnConnStart(conn isocket.IConnection) {
...
func (s *Server) CallOnConnStop(conn isocket.IConnection) {
	if s.OnConnStop != nil {
